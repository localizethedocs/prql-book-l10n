...
message(STATUS "Running 'mdbook build' command with 'xgettext' renderer to generate .pot files...")
if (CMAKE_HOST_LINUX)
...
block(PROPAGATE MDBOOK_PREPROCESSOR)
    ...
    if (VERSION VERSION_EQUAL "0.1")
        set(MDBOOK_PREPROCESSOR__PRQL "{}")
        string(JSON MDBOOK_PREPROCESSOR__PRQL SET "${MDBOOK_PREPROCESSOR__PRQL}" "command" "\"cargo run --bin mdbook-prql\"")
        string(JSON MDBOOK_PREPROCESSOR SET       "${MDBOOK_PREPROCESSOR}" "prql" "${MDBOOK_PREPROCESSOR__PRQL}")
        string(JSON MDBOOK_PREPROCESSOR REMOVE    "${MDBOOK_PREPROCESSOR}" "install-prql")
    endif()
endblock()
